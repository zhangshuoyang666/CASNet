...
# Best run source (searched from all metrics.tsv):
# workdirs/exp_spatial_cbam_saff_boundaryaux_continue50_es5_step/metrics.tsv
# best mIoU = 0.947726
SOURCE_BEST_METRICS="${PROJECT_DIR}/workdirs/exp_spatial_cbam_saff_boundaryaux_continue50_es5_step/metrics.tsv"
SOURCE_BEST_MIOU="0.947726"

# Keep early-stop behavior from best run naming ("es5")
EARLY_STOP_PATIENCE="${6:-5}"
EARLY_STOP_MIN_DELTA="${7:-0.0001}"
MIN_ITRS_BEFORE_ES="${8:-1000}"

# Adaptive LR by linear scaling from bs16/lr0.01
LR="$(python - <<PY
base_lr=${BASE_LR}
batch=${BATCH_SIZE}
...
EXP_NAME="bestcfg_nonbaseline_bs${BATCH_SIZE}_lr${LR_TAG}_es${EARLY_STOP_PATIENCE}_md${DELTA_TAG}_${TIMESTAMP}"
...
{
  "exp_name": "${EXP_NAME}",
  "profile": "best_nonbaseline_from_global_search",
  "source_best_metrics": "${SOURCE_BEST_METRICS}",
  "source_best_miou": ${SOURCE_BEST_MIOU},
  "dataset": "cottonweed",
  "data_root": "${DATA_ROOT}",
...
cat > "${BEST_DIR}/latest_bestcfg_bs8_run.txt" <<EOF
exp_name=${EXP_NAME}
run_dir=${RUN_DIR}
pid=${PID}
batch_size=${BATCH_SIZE}
...
echo "Started best-config non-baseline bs=${BATCH_SIZE} training with nohup."
echo "PID: ${PID}"
