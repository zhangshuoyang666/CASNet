#!/usr/bin/env bash
###############################################################################
# SA-BNet Serial Hyperparameter Optimization v3
#
# Usage:
#   nohup bash auto_optimize.sh > auto_optimize.log 2>&1 &
#   echo $! > auto_optimize_pid.txt
#   tail -f auto_optimize.log
###############################################################################
set -euo pipefail
...
build_args() {
  ...
  local itrs="$8" vi="$9" workdir="${10}" bs="${11:-$BEST_BS}" aspp_var="${12:-$BEST_ASPP_VARIANT}"
  ...
  a+=" --batch_size $bs --val_batch_size $VAL_BS --random_seed 1"
  a+=" --lr $lr --weight_decay $wd --dense_aspp_rates $aspp --aspp_variant $aspp_var"
  a+=" --total_itrs $itrs --val_interval $vi --print_interval $PRINT_INTERVAL"
...
EOFSH
  ...
  if [ -d "$PROJECT/.git" ]; then
    git_hash=$(cd "$PROJECT" && git rev-parse --short HEAD 2>/dev/null || echo "N/A")
  fi
...
EOFS
  ...
  cd "$PROJECT"
  python -u main.py $args > "$dir/train.log" 2>&1
  local ecode=$?
  echo "ended: $(date '+%F %T')" >> "$dir/status.txt"
  echo "exit_code: $ecode" >> "$dir/status.txt"

  if [ $ecode -ne 0 ]; then
    if grep -q "OutOfMemoryError\|CUDA out of memory" "$dir/train.log" 2>/dev/null; then
      echo "OOM_DETECTED" >> "$dir/status.txt"
      log "✗ OOM detected in $name" >&2
      return 2
    fi
  fi

  python "$PARSE" "$dir" "$name" >&2 || true
  log "✓ DONE  $name (exit=$ecode)" >&2
  return 0
}

###############################################################################
# get_best_from_csv: extract best mIoU → "miou|abuth|cotton|iter|epoch"
###############################################################################
get_best_from_csv() {
  local csv="$1"
...
###############################################################################
# prep_dir: create experiment directory + config_snapshot
###############################################################################
prep_dir() {
...
###############################################################################
# run_and_log: run one candidate, parse, return miou string
###############################################################################
run_and_log() {
  local dir="$1" args="$2"
  run_exp "$dir" "$args"
...
###############################################################################
# OOM fallback: if bs=32 OOMs, reduce to 16
###############################################################################
handle_oom_fallback() {
  if [ "$BEST_BS" = "32" ]; then
    log "⚠ OOM with bs=32. Falling back to bs=16."
    BEST_BS="16"
    update_best_json "baseline" "oom_fallback_bs16"
  fi
}
...
###############################################################################
log "================================================================"
log "  STEP 1: Learning Rate Screening  (bs=$BEST_BS)"
log "================================================================"
...
for lr_pair in "0p001:0.001" "0p005:0.005" "0p01:0.01" "0p02:0.02"; do
  ...
  DIR="$BASE/01_lr_${tag}_bs${BEST_BS}"
...
if [ "$STEP1_COMPLETE" = true ]; then
  log "★ Step 1 已完成，复用已有结果"
  for lr_pair in "0p001:0.001" "0p005:0.005" "0p01:0.01" "0p02:0.02"; do
    IFS=: read tag lr <<< "$lr_pair"
    DIR="$BASE/01_lr_${tag}_bs${BEST_BS}"
    RESULT=$(get_best_from_csv "$DIR/metrics.csv")
    LR_RES["$tag"]="$RESULT"
    IFS='|' read m a c it ep <<< "$RESULT"
    log "  [cached] lr=$lr → mIoU=${m}% abuth=${a}% cotton=${c}% @iter=$it"
  done
else
  for lr_pair in "0p001:0.001" "0p005:0.005" "0p01:0.01" "0p02:0.02"; do
    IFS=: read tag lr <<< "$lr_pair"
    ...
    prep_dir "$DIR" "1_lr" "lr" "0.01" "$lr" "Step1: lr=$lr bs=$BEST_BS screen ${SHORT_ITRS}itr"
    ARGS=$(build_args "$lr" "$BEST_WD" "$BEST_LAMBDA" "0" "" "0" "$BEST_ASPP" "$SHORT_ITRS" "$VAL_INTERVAL" "$DIR" "$BEST_BS")
    RESULT=$(run_and_log "$DIR" "$ARGS") || true

    if [[ "$RESULT" == OOM* ]]; then
      OOM_HIT=true
      log "⚠ OOM on first LR candidate with bs=$BEST_BS"
      break
    fi
  ...
  if [ "$OOM_HIT" = true ]; then
    handle_oom_fallback
    log "Restarting LR screening with bs=$BEST_BS"
    unset LR_RES
    declare -A LR_RES
    ...
    for lr_pair in "0p001:0.001" "0p005:0.005" "0p01:0.01" "0p02:0.02"; do
      ...
      RESULT=$(run_and_log "$DIR" "$ARGS") || true
      if [[ "$RESULT" == OOM* ]]; then
        log "✗ FATAL: OOM even with bs=16. Aborting."
        exit 1
      fi
      LR_RES["$tag"]="$RESULT"
      IFS='|' read m a c it ep <<< "$RESULT"
...
case "$BEST_LR_TAG" in 0p001) BLR=0.001;; 0p005) BLR=0.005;; 0p01) BLR=0.01;; 0p02) BLR=0.02;; *) BLR=0.01; log "⚠ No LR winner, defaulting to 0.01";; esac
log "★ Best LR from screen: $BLR (mIoU=$BEST_LR_MIOU%)"

python "$COMPARE" \
  "$BASE/01_lr_0p001_bs${BEST_BS}" "$BASE/01_lr_0p005_bs${BEST_BS}" \
  "$BASE/01_lr_0p01_bs${BEST_BS}" "$BASE/01_lr_0p02_bs${BEST_BS}" \
  --output "$BASE/step1_lr_comparison.png" \
  --title "Step1 LR Screen (bs${BEST_BS}, 800itr)" 2>&1 || true

for lr_pair in "0p001:0.001" "0p005:0.005" "0p01:0.01" "0p02:0.02"; do
  IFS=: read tag lr <<< "$lr_pair"
  IFS='|' read m a c it ep <<< "${LR_RES[$tag]}"
  gain=$(python3 -c "print(f'{float(\"$m\")-float(\"$BEST_MIOU\"):.2f}')" 2>/dev/null || echo "0")
  dec=$( [ "$tag" = "$BEST_LR_TAG" ] && echo "promote_to_confirm" || echo "reject" )
  append_tracker "1,01_lr_${tag}_bs${BEST_BS},baseline,lr,0.01,$lr,$SHORT_ITRS,,$m,$it,$a,$c,$gain,$dec,,01_lr_${tag}_bs${BEST_BS}"
done
...
CDIR="$BASE/01_confirm_lr_${BEST_LR_TAG}_bs${BEST_BS}"
if [ -f "$CDIR/metrics.csv" ]; then
  log "  [cached] Step 1 confirm 已完成，复用结果"
  RESULT=$(get_best_from_csv "$CDIR/metrics.csv")
else
  prep_dir "$CDIR" "1c" "lr" "0.01" "$BLR" "Step1 confirm: lr=$BLR full ${CONFIRM_ITRS}itr"
  ARGS=$(build_args "$BLR" "$BEST_WD" "$BEST_LAMBDA" "0" "" "0" "$BEST_ASPP" "$CONFIRM_ITRS" "$VAL_INTERVAL" "$CDIR" "$BEST_BS")
  RESULT=$(run_and_log "$CDIR" "$ARGS") || true
fi
IFS='|' read m a c it ep <<< "$RESULT"
...
if [ "$CMP" != "worse" ]; then
  ...
  append_tracker "1c,confirm_lr_${BEST_LR_TAG},baseline,lr,0.01,$BLR,$SHORT_ITRS,$CONFIRM_ITRS,$m,$it,$a,$c,$gain,$DEC,,01_confirm_lr_${BEST_LR_TAG}_bs${BEST_BS}"
  log "★ Best updated: lr=$BEST_LR mIoU=$BEST_MIOU%"
else
  append_tracker "1c,confirm_lr_${BEST_LR_TAG},baseline,lr,0.01,$BLR,$SHORT_ITRS,$CONFIRM_ITRS,$m,$it,$a,$c,$gain,reject,not better,01_confirm_lr_${BEST_LR_TAG}_bs${BEST_BS}"
  BEST_LR="0.01"
...
ARGS=$(build_args "$BEST_LR" "$BEST_WD" "$BEST_LAMBDA" "300" "$BEST_VFLIP" "$BEST_ROTATION" "$BEST_ASPP" "$SHORT_ITRS" "$VAL_INTERVAL" "$WDIR" "$BEST_BS")
RESULT=$(run_and_log "$WDIR" "$ARGS") || true
IFS='|' read m a c it ep <<< "$RESULT"
gain=$(python3 -c "print(f'{float(\"$m\")-float(\"$BEST_MIOU\"):.2f}')")
CMP=$(compare_miou "$m" "$BEST_MIOU")
...
ARGS=$(build_args "$BEST_LR" "$BEST_WD" "$BEST_LAMBDA" "$BEST_WARMUP" "yes" "$BEST_ROTATION" "$BEST_ASPP" "$SHORT_ITRS" "$VAL_INTERVAL" "$VDIR" "$BEST_BS")
RESULT=$(run_and_log "$VDIR" "$ARGS") || true
IFS='|' read m a c it ep <<< "$RESULT"
gain=$(python3 -c "print(f'{float(\"$m\")-float(\"$BEST_MIOU\"):.2f}')")
CMP=$(compare_miou "$m" "$BEST_MIOU")
...
ARGS=$(build_args "$BEST_LR" "$BEST_WD" "$BEST_LAMBDA" "$BEST_WARMUP" "$BEST_VFLIP" "30" "$BEST_ASPP" "$SHORT_ITRS" "$VAL_INTERVAL" "$RDIR" "$BEST_BS")
RESULT=$(run_and_log "$RDIR" "$ARGS") || true
IFS='|' read m a c it ep <<< "$RESULT"
gain=$(python3 -c "print(f'{float(\"$m\")-float(\"$BEST_MIOU\"):.2f}')")
CMP=$(compare_miou "$m" "$BEST_MIOU")
...
declare -A LAM_RES
BEST_LAM_MIOU="0"; BEST_LAM_TAG=""; BEST_LAM_VAL=""; BEST_LAM_RES=""
for lp in "0p2:0.2" "0p3:0.3" "0p4:0.4"; do
  ...
  ARGS=$(build_args "$BEST_LR" "$BEST_WD" "$lval" "$BEST_WARMUP" "$BEST_VFLIP" "$BEST_ROTATION" "$BEST_ASPP" "$SHORT_ITRS" "$VAL_INTERVAL" "$LDIR" "$BEST_BS")
  RESULT=$(run_and_log "$LDIR" "$ARGS") || true
  LAM_RES["$ltag"]="$RESULT"
  IFS='|' read m a c it ep <<< "$RESULT"
  log "  λ=$lval → mIoU=${m}%"
...
if [ -n "$BEST_LAM_RES" ]; then
  ...
  for lp in "0p2:0.2" "0p3:0.3" "0p4:0.4"; do
    IFS=: read ltag lval <<< "$lp"
    mm="0"; aa="0"; cc="0"; ii="0"; ee="0"
    if [ -n "${LAM_RES[$ltag]:-}" ]; then
      IFS='|' read mm aa cc ii ee <<< "${LAM_RES[$ltag]}"
    fi
...
ARGS=$(build_args "$BEST_LR" "5e-4" "$BEST_LAMBDA" "$BEST_WARMUP" "$BEST_VFLIP" "$BEST_ROTATION" "$BEST_ASPP" "$SHORT_ITRS" "$VAL_INTERVAL" "$WDDIR" "$BEST_BS")
RESULT=$(run_and_log "$WDDIR" "$ARGS") || true
IFS='|' read m a c it ep <<< "$RESULT"
gain=$(python3 -c "print(f'{float(\"$m\")-float(\"$BEST_MIOU\"):.2f}')")
CMP=$(compare_miou "$m" "$BEST_MIOU")
...
declare -A ASPP_RES
BEST_ASPP_MIOU="0"; BEST_ASPP_TAG=""; BEST_ASPP_VAL=""; BEST_ASPP_RES=""
for ap in "1_6_12_18_24:1,6,12,18,24" "1_2_4_8_16:1,2,4,8,16"; do
  IFS=: read atag aval <<< "$ap"
  ADIR="$BASE/07_aspp_${atag}"
  prep_dir "$ADIR" "6" "aspp_rates" "$BEST_ASPP($BEST_ASPP_VARIANT)" "$aval(dense)" "Step6: DenseASPP rates=($aval)"
  ARGS=$(build_args "$BEST_LR" "$BEST_WD" "$BEST_LAMBDA" "$BEST_WARMUP" "$BEST_VFLIP" "$BEST_ROTATION" "$aval" "$SHORT_ITRS" "$VAL_INTERVAL" "$ADIR" "$BEST_BS" "dense")
  RESULT=$(run_and_log "$ADIR" "$ARGS") || true
  ASPP_RES["$atag"]="$RESULT"
  IFS='|' read m a c it ep <<< "$RESULT"
  log "  DenseASPP=($aval) → mIoU=${m}%"
...
if [ -n "$BEST_ASPP_RES" ]; then
  ...
  for ap in "1_6_12_18_24:1,6,12,18,24" "1_2_4_8_16:1,2,4,8,16"; do
    IFS=: read atag aval <<< "$ap"
    mm="0"; aa="0"; cc="0"; ii="0"; ee="0"
    if [ -n "${ASPP_RES[$atag]:-}" ]; then
      IFS='|' read mm aa cc ii ee <<< "${ASPP_RES[$atag]}"
    fi
    gg=$(python3 -c "print(f'{float(\"$mm\")-float(\"$BEST_MIOU\"):.2f}')" 2>/dev/null || echo "0")
  ...
  if [ "$CMP" != "worse" ]; then
    BEST_MIOU="$m"; BEST_ASPP="$BEST_ASPP_VAL"; BEST_ASPP_VARIANT="dense"
    update_best_json "6" "aspp_$BEST_ASPP_TAG"
    log "★ Best updated: aspp=$BEST_ASPP (dense variant)"
  else
...
IMPROVED_LIST=$(python3 -c "
import csv
with open('$TRACKER') as f:
    rows = list(csv.reader(f))
improved = []
for r in rows[1:]:
    if len(r) >= 14 and r[13] in ('update_best', 'marginal_gain'):
        improved.append(f'  - {r[1]}: mIoU={r[8]}% (gain={r[12]})')
if improved:
    print('\n'.join(improved))
else:
    print('  (none)')
...
log "Final best: mIoU=$BEST_MIOU% lr=$BEST_LR wd=$BEST_WD λ=$BEST_LAMBDA warmup=$BEST_WARMUP vflip=$([ -n "$BEST_VFLIP" ] && echo Y || echo N) rot=$BEST_ROTATION aspp=$BEST_ASPP bs=$BEST_BS"
