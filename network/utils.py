...
class _SimpleSegmentationModel(nn.Module):
    ...
    def forward(self, x):
        ...
        if isinstance(x, dict):
            seg = F.interpolate(x["seg_logits"], size=input_shape, mode='bilinear', align_corners=False)
            output = {"out": seg}
            if "edge_logits" in x:
                output["edge"] = F.interpolate(x["edge_logits"], size=input_shape, mode='bilinear', align_corners=False)
            return output
